...
public class ProductRepository : IProductRepository
{
    ...
    public async Task<IEnumerable<Product>> GetProductsAsync() 
        => await _context.Products.Include(c => c.ProductCategory).ToListAsync();

    public async Task<Product?> GetProductByIdAsync(int id)  => await _context.Products.FindAsync(id);

    public async Task<Product?> GetProductByProductNumberAsync(string number) => await _context.Products.FirstOrDefaultAsync(p => p.ProductNumber == number);

    public async Task<Product?> GetProductByNameAsync(string name) 
        => await _context.Products.FirstOrDefaultAsync(p => p.ProductName == name);

    public async Task CreateProductAsync(Product product) => await _context.AddAsync(product);
